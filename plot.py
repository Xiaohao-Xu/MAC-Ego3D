...
if __name__ == '__main__':
    ...
    path_cpslam_1 = "/home/jyuzhu/CP-SLAM/cp-slam/experiments/redkitchen/pgo_traj_1.pt"
    ckpt_cpslam_1 = torch.load(path_cpslam_1, map_location=torch.device('cpu'))
    poses_gt_1 = torch.zeros_like(ckpt_cpslam_1)
    with open("/media/hdd/xiaohao/data/7Scene_ICP/redkitchen/seq-01/traj.txt") as f:
    # with open("/media/hdd/xiaohao/data/cpslam_data/cp-slam/Apart-1/apart_1_part1/traj.txt") as f:
        poses=f.readlines()
        index = 0
        for line in poses:
            ...
            poses_gt_1[index] = T
            index += 1
    poses_gt_1,mask = convert_poses(poses_gt_1,poses_gt_1.shape[0]-1,1)
    ckpt_cpslam_1, _ = convert_poses(ckpt_cpslam_1,ckpt_cpslam_1.shape[0]-1,1)
    rot, trans, trans_error, ckpt_cpslam_1 = align(np.matrix(ckpt_cpslam_1[:,0:3]).T, np.matrix(poses_gt_1[:,0:3]).T)
    
    path_cpslam_2 = "/home/jyuzhu/CP-SLAM/cp-slam/experiments/redkitchen/pgo_traj_2.pt"
    ckpt_cpslam_2 = torch.load(path_cpslam_2, map_location=torch.device('cpu'))
    poses_gt_2 = torch.zeros_like(ckpt_cpslam_2)
    with open("/media/hdd/xiaohao/data/7Scene_ICP/redkitchen/seq-03/traj.txt") as f:
    # with open("/media/hdd/xiaohao/data/cpslam_data/cp-slam/Apart-1/apart_1_part2/traj.txt") as f:
        poses=f.readlines()
        index = 0
        for line in poses:
            T = np.array(list(map(float, line.split()))).reshape(4, 4)
            ...
            poses_gt_2[index] = T
            index += 1
    poses_gt_2,mask = convert_poses(poses_gt_2,poses_gt_2.shape[0]-1,1)
    ckpt_cpslam_2, _ = convert_poses(ckpt_cpslam_2,ckpt_cpslam_2.shape[0]-1,1)
    rot, trans, trans_error, ckpt_cpslam_2 = align(np.matrix(ckpt_cpslam_2[:,0:3]).T, np.matrix(poses_gt_2[:,0:3]).T)
    
    path_our_1 = "/media/hdd/xiaohao/experiments_ma_7scene/results_test/redkitchen_1/Agent0_est.pt"
    # path_our_1 = "/media/hdd/xiaohao/experiments_ma_ulim/results/Apart-1/Agent0_est.pt"
    ckpt_our_1 = torch.load(path_our_1, map_location=torch.device('cpu'))
    
    path_our_2 = "/media/hdd/xiaohao/experiments_ma_7scene/results_test/redkitchen_1/Agent1_est.pt"
    # path_our_2 = "/media/hdd/xiaohao/experiments_ma_ulim/results/Apart-1/Agent1_est.pt"
    ckpt_our_2 = torch.load(path_our_2, map_location=torch.device('cpu'))
    
    poses_gt_1 = poses_gt_1[:,0:2]
    poses_gt_2 = poses_gt_2[:,0:2]
    # poses_gt_1 = poses_gt_1.T
    # poses_gt_2 = poses_gt_2.T
    ckpt_cpslam_1 = ckpt_cpslam_1.T
    ckpt_cpslam_2 = ckpt_cpslam_2.T
    ckpt_our_1 = ckpt_our_1.numpy().T
    ckpt_our_2 = ckpt_our_2.numpy().T
    
    print(poses_gt_1.shape,poses_gt_2.shape)
    pyplot.clf()
    pyplot.title(f'redkitchen')
    pyplot.plot(poses_gt_1[:,0], poses_gt_1[:,1], label='GT_1', linewidth=3)
    pyplot.legend()
    pyplot.plot(poses_gt_2[:,0], poses_gt_2[:,1], label='GT_2', linewidth=3)
    pyplot.legend()
    pyplot.plot(ckpt_cpslam_1[:,0], ckpt_cpslam_1[:,1], label='CPSLAM_1_est', linewidth=2 )
    pyplot.legend()
    pyplot.plot(ckpt_our_1[:,0], ckpt_our_1[:,1], label='MAGA_1_est', linewidth=2)
    pyplot.legend()
    pyplot.plot(ckpt_cpslam_1[:,0], ckpt_cpslam_2[:,1], label='CPSLAM_2_est', linewidth=2)
    pyplot.legend()
    pyplot.plot(ckpt_our_2[:,0], ckpt_our_2[:,1], label='MAGA_2_est', linewidth=2)
    pyplot.legend()
    # pyplot.axis('equal')
    pyplot.savefig('Traj_heads.jpg',dpi=500)
