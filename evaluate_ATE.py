...
if __name__ == '__main__':
    ...
    if os.path.exists(ckptsdir):
        ckpts = ["/home/jyuzhu/CP-SLAM/cp-slam/experiments/room2/pgo_traj_1.pt"]
        # ckpts = ["/home/jyuzhu/CP-SLAM/cp-slam/experiments/heads/pgo_traj_2.pt"]
        if len(ckpts) > 0:
            ckpt_path = ckpts[-1]
            ...
            # print(ckpt)
            poses_est = ckpt
            poses_gt = torch.zeros_like(poses_est)
            # with open("/media/hdd/xiaohao/data/7Scene_ICP/heads/seq-02/traj.txt") as f:
            with open("/media/hdd/xiaohao/data/replica/room2/traj.txt") as f:
                poses=f.readlines()
                index = 0
                for line in poses:
            ...
            poses_gt,mask = convert_poses(poses_gt,poses_gt.shape[0]-1,scale)
            poses_est, _ = convert_poses(poses_est,poses_est.shape[0]-1,scale)
            print(poses_gt, poses_est.shape) 
